...
package object ui {
  ...
  implicit class GraphicsExt(private val graphics: Graphics2D) extends AnyVal {
    ...
    def getTextRendering(text: String, font: Font, hAlign: HAlign, vAlign: VAlign): TextRendering =
      TextRendering.calculate(graphics, text, font, hAlign, vAlign)

    def getReducedTextRendering(text: String, font: Font, hAlign: HAlign): TextRendering = {
      ...
      def rec(fragment: String, isFull: Boolean): TextRendering = if (fragment.nonEmpty) {
        ...
        if (stringBounds.getWidth <= clipBounds.getWidth)
          graphics.getTextRendering(toPrint, font, hAlign = hAlign, VAlign.Center)
        else
  ...
  implicit class Rectangle2DExt(private val rectangle: Rectangle2D)
    extends AnyVal {

    def &(right: Rectangle2D): Rectangle2D =
      rectangle createIntersection right
  }
}
